...
@pytest.mark.withartiq
def test_mixed_dag_prepare_ops_allows_host_dep(fragment_factory):
    """A kernel primary can build its kernel ops over a mixture of kernel and
    host-only deps: only the kernel-checked nodes are embedded in the driver."""
    top = fragment_factory(cals.MixedKernelTop)
    top.host_setup()

    deps = top._get_dependencies()
    kernel_flags = {
        dep.__class__.__name__: flag for dep, flag in zip(deps, top._csk_is_kernel)
    }
    assert kernel_flags["MixedKernelDep"] is True
    assert kernel_flags["MixedHostDep"] is False
    assert kernel_flags["MixedKernelTop"] is True
