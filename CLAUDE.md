...
When a node's fix attempt fails, the real culprit is often a dependency that still *looks* good — checked recently, inside its timeout — but has silently drifted. Each such **direct** dependency is marked **suspect**: the dependent's class name is added to its `suspected_by` set (persisted in `calibrations.status`, restored by `_recall_status`). Suspicion travels **one edge per failure** (Charles, 2026-08-02): a failure in C accuses only C's own dependencies; the next hop is accused only if a direct dependency itself then fails to come good, so the cascade reaches deep ancestors hop by hop through the walk's re-planning rather than one failure smearing doubt over the whole chain. (Retraction on success deliberately stays transitive — it is a no-op on non-suspects and cleans up pre-one-hop marks.) SUSPECT is deliberately **not** a `CalibrationResult` member — a check can't *measure* "suspect"; it is trust metadata, and `check_own_state` can never return it.
